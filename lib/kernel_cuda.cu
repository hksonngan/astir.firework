...
// Update the volume
__global__ void	pet3D_OPLEM_update_V0(float* d_im, unsigned int* d_F, float* d_NM, float invscale, int nim) {
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx < nim) {
...
// DDA ray-projector
__global__ void pet3D_OPLEM_DDA_V0(unsigned int* d_F, int sublor_start, int sublor_stop, int nim3,
...
void kernel_pet3D_OPLEM_wrap_cuda_V0(unsigned short int* x1, int nx1, unsigned short int* y1, int ny1,
	...
	for (int iouter=0; iouter < Nouter; ++iouter) {
		...
		for (int isub=0; isub < nsub; ++isub) {
			...
			threads.x = block_size;
			grid.x = grid_size;
			pet3D_OPLEM_DDA_V0<<<grid, threads>>>(d_F, sublor_start, sublor_stop, nim3,
...
__global__ void pet3D_OPLEM_DDA_V1(unsigned int* d_F, int sublor_start, int sublor_stop, int nim3,
								   int step, int nim, int nsublor, float scale) {
...
void kernel_pet3D_OPLEM_wrap_cuda_V1(unsigned short int* x1, int nx1, unsigned short int* y1, int ny1,
									 unsigned short int* z1, int nz1, unsigned short int* x2, int nx2,
									 unsigned short int* y2, int ny2, unsigned short int* z2, int nz2,
									 float* im, int nim1, int nim2, int nim3,
	...
	for (int iouter=0; iouter < Nouter; ++iouter) {
		...
		for (int isub=0; isub < nsub; ++isub) {
			...
			threads.x = block_size;
			grid.x = grid_size;
			pet3D_OPLEM_DDA_V1<<<grid, threads>>>(d_F, sublor_start, sublor_stop, nim3,
												  nim3*nim3, nim, nsublor, scale);
			// Unbind im tex
			cudaUnbindTexture(tex_im);
